...
def run_quad_fdm(conf: Namespace, _run=None):
    ...
    for optim in optim_list:
        run_quad_fdm_with_optim(conf, optim, _run)

def run_quad_fdm_with_optim(conf: Namespace, optimizer, _run=None):
    # seeding
    ...
    elif conf.pipeline == 'seq':
        # two stage, discrete with 1+1 with trim score then continuous with CMA with raw score
        param2 = ng.p.Dict()
        param2['lqr_vector1'] = ng.p.Array(shape=(conf.design_space['LQR_1'][0], ), lower=conf.design_space['LQR_1'][1], upper=conf.design_space['LQR_1'][2])
        param2['lqr_vector3'] = ng.p.Array(shape=(conf.design_space['LQR_3'][0], ), lower=conf.design_space['LQR_3'][1], upper=conf.design_space['LQR_3'][2])
        param2['lqr_vector4'] = ng.p.Array(shape=(conf.design_space['LQR_4'][0], ), lower=conf.design_space['LQR_4'][1], upper=conf.design_space['LQR_4'][2])
        param2['lqr_vector5'] = ng.p.Array(shape=(conf.design_space['LQR_5'][0], ), lower=conf.design_space['LQR_5'][1], upper=conf.design_space['LQR_5'][2])
        # TODO
        # latvel should be from max latvel from trim output
        # vertvel should be fixed values
        param2['lat_vel'] = ng.p.Array(shape=(conf.design_space['lateral_velocity'][0], ), lower=conf.design_space['lateral_velocity'][1], upper=conf.design_space['lateral_velocity'][2])
        param2['vert_vel'] = ng.p.Array(shape=(conf.design_space['vertical_velocity'][0], ), lower=conf.design_space['vertical_velocity'][1], upper=conf.design_space['vertical_velocity'][2])
