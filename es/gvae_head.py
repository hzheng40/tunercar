...
from gvae_worker import GVAEWorker

def run_gvae_fdm(conf: Namespace, _run=None):
    # seeding
    np.random.seed(conf.seed)

    # saving the optimizer
    import os
    if not os.path.exists('gvae_fdm_runs/npzs'):
        os.makedirs('gvae_fdm_runs/npzs')
    if not os.path.exists('gvae_fdm_runs/optims_pkl'):
        os.makedirs('gvae_fdm_runs/optims_pkl')
    filename = 'gvae_fdm_runs/npzs/' + conf.run_name + '_' + conf.optim_method + '_budget' + str(conf.budget) + '.npz'
    filename_optim = 'gvae_fdm_runs/optims_pkl/' + conf.run_name + '_' + conf.optim_method + '_budget' + str(conf.budget) + '_optim.pkl'
